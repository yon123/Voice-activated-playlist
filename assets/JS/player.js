 var audioPlayer = {
  ...
  load: function() {
    ...
    data.songs.forEach(function(val, i) {
      $("#playlist").append(
        "<li class='list-group-item'>" + val.artist + " - " + val.songName
      );
    })
  },
  changeCurrentSongEffect: function(options) {
    if (options.play) {
      ...
      $("#playlist .list-group-item")
        ...
        .append("<span class='glyphicon glyphicon-sunglasses'>");
    }
    if (options.end) {
      $("#playlist .list-group-item")
      .eq(this.audioData.currentSong)
      .removeClass("list-group-item-success glyphicon-sunglasses")
    ...
    try {
      this.data.forEach(function(val, i) {
        if (val.songName.trim().toLowerCase().indexOf(keyword) !== -1 ||
            val.artist.trim().toLowerCase().indexOf(keyword) !== -1) {
